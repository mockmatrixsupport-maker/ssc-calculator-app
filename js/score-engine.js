...
const RSMScoreEngine = (() => {
  ...
  function shortSectionName(name) {
    ...
    if (SECTION_SHORT_MAP[key]) return SECTION_SHORT_MAP[key];
    const words = clean.split(/\s+/).filter(Boolean);
    if (words.length <= 1) return clean.length > 10 ? clean.slice(0, 9) + '…' : clean;
    if (words.length === 2) return clean.length > 12 ? clean.slice(0, 11) + '…' : clean;
    // 3+ words and no known mapping — acronym from initials, e.g.
  ...
  function buildMarksTable(r) {
    ...
    const bodyRows = r.sections.map(s => `
      <tr>
        <td class="mt-col-section" title="${esc(s.name)}">${esc(shortSectionName(s.name))}</td>
        <td>${s.total}</td>
